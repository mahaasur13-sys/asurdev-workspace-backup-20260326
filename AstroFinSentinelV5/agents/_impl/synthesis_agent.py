    ...
    async def run(self, state: dict) -> AgentResponse:
        ...
        try:
            from agents._impl.amre import (
                MarketState, trajectory_from_state, compute_trajectory_reward,
                estimate_q_star, estimate_uncertainty, validate_with_grounding,
                SelfQuestioningEngine, HierarchicalPolicy, CounterfactualEngine,
                select_ensemble, OAPOptimizer, record_decision
            )
            ms = MarketState(
                symbol=symbol,
                price=current_price,
            ...
            oap_result = oap.validate_and_adjust(
                {"uncertainty": uncertainty, "q_star": vol_risk.kelly_adjusted if vol_risk else 0.5, "regime": regime.value if hasattr(regime, "value") else "NORMAL", "timestamp": datetime.now().isoformat()},
                confidence, risk_pct
            )
            amre_meta = {
                "enabled": True,
            ...
            meta["oap_validation"] = {"status": oap_result.status.value, "issues": oap_result.issues}
            record_decision(
